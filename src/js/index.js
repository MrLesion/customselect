    ...
    $.fn.customselect = function ( options ) {
        ...
        const customSelect = {
            ...
            bindByElement: ( domSelect, boolInit = false, domSelector = null ) => {
                ...
                const optGroups = Array.from( domSelect.children ).filter( o => o.nodeName === 'OPTGROUP' );
                ...
                if ( optGroups.length > 0 ) {
                    ...
                    Array.from( optGroups ).forEach( ( optgroup ) => {
                        const optionsInGroup = Array.from( optgroup.children );
                        optionsInGroup.forEach( ( option ) => {
                            let nestedOption = domCheckboxList.querySelector( 'input[value="' + option.value + '"]' ).closest( '.customselect-list-input-item' );
                            console.log( nestedOption );
                            nestedList.appendChild( nestedOption );
                            //domCheckboxList.removeChild( nestedOption );
                        } );
                    } );
